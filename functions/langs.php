					...
					if(($lang = json_decode($json, true)) !== null) {
					    ...
					    switch (json_last_error()) {
					        ...
					        case JSON_ERROR_DEPTH:
					            trigger_error('Json decode error: ' . $file . ' - Maximum stack depth exceeded');
					        break;
					        case JSON_ERROR_STATE_MISMATCH:
					            trigger_error('Json decode error: ' . $file . ' - Underflow or the modes mismatch');
					        break;
					        case JSON_ERROR_CTRL_CHAR:
					            trigger_error('Json decode error: ' . $file . ' - Unexpected control character found');
					        break;
					        case JSON_ERROR_SYNTAX:
					            trigger_error('Json decode error: ' . $file . ' - Syntax error, malformed JSON');
					        break;
					        case JSON_ERROR_UTF8:
					            trigger_error('Json decode error: ' . $file . ' - Malformed UTF-8 characters, possibly incorrectly encoded');
					        break;
					        default:
					            trigger_error('Json decode error: ' . $file . ' - Unknown error');
					        break;
					    }
					}
				}
